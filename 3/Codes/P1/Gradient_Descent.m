...
nhid=70; % Number of Hidden Layer Neurons
input_layer=zeros(784,60000);
output_layer=zeros(10,60000);
hidden_layer=zeros(nhid,60000);

%%Weight matrix initialization
Vv=rand(nhid,784);
prev=zeros(nhid,784);
Ww=rand(10,nhid);   %weight matrix of hidden_layer and output_layer Neurons
prew=zeros(10,nhid);
...
for i=1:1:784
    ...
    trainx(:,i)=double((trainX(:,i)))/double((max(trainX(:,i))-min(trainX(:,i))));
    else
         trainx(:,i)=double(trainX(:,i));
    end
end
trainx=trainx/2;
...
for iteration=1:1:400%I see this from a site with 0.83 precesion  for iteration=1:1:15000%be nazaram felan yebaresh kon bebin natije chie badan tedado ziad kon
    ...
    %An epoch for all data 
        ...
        delta_out=output_layer.*(ones(10,60000)-output_layer).*(t-output_layer);

        %compute delta hidden layer
        delta_hidden=w'*delta_out.*(hidden_layer.*(ones(nhid,60000)-hidden_layer));
    
        %compute weight change for w matrix
       delta_w=double((delta_out*hidden_layer'))/60000;
    
        %compute weight change for v matrix
        delta_v=double((delta_hidden*input_layer'))/60000;
    ...
    if(iteration>310)  
     ...
     v=v+eta*delta_v+0.01*prev;
     w=w+eta*delta_w+0.01*prew;
     prev=delta_v;
     prew=delta_w;
 ...
 for j=1:1:10
  ...
  for rt=1:1:60000
   ...
   accuracy_train3(iteration)=accuracy_train2/60000;

function [tree] = P1c_ID3(examples, attributes, activeAttributes)
...
currentEntropy= Entropy( lastColumnSum,numberExamples );
rr=length(find(activeAttributes==0));
best=Best_attr( currentEntropy,activeAttributes,examples );
activeAttributes_mid=activeAttributes;
if(rr>0)
bestAttribute=best;
else
activeAttributes_mid(best) = 0;    
bestAttribute=Best_attr( currentEntropy,activeAttributes_mid,examples );
end

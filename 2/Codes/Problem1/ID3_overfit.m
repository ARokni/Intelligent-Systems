function [tree] = ID3_overfit(examples, attributes, activeAttributes)
number_of_layer=2;
...
bestAttribute=Best_attr( currentEntropy,activeAttributes,examples );
...
over_fit_condition=find(activeAttributes==0);
over_fit_condition_number=length(over_fit_condition);
if (over_fit_condition_number == number_of_layer);
    if (lastColumnSum >= numberExamples / 2);
        tree.value = 'true';
    else
        tree.value = 'false';
